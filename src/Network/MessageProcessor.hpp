...
public:
    ...
    void processMessage(std::string &connectionID, MessageShPtr message);
...
private:
    ...
    /// @todo priority queue!
    std::queue<std::pair<std::string, MessageShPtr>> processingQueue; ///< Elements are pairs of connection identifier and message that should be processed
    ...
    void addMessageToProcessingQueue(std::string &connectionID, MessageShPtr message);
